...
export default {
  ...
  methods: {
     ...
     */
    loadData: debounce(function(pagination, filters, sorter) {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          this.localPagination =
            (this.showPagination &&
              Object.assign({}, this.localPagination, {
                current: r.meta.current_page, // 返回结果中的当前分页数
                total: r.meta.total, // 返回结果中的总记录数
                showSizeChanger: this.showSizeChanger,
                pageSize: (pagination && pagination.per_page) || this.localPagination.pageSize
              })) ||
            false
        ...
        return (
          <a-alert showIcon={true} style="margin-bottom: 16px">
            <template slot="message">
              <span style="margin-right: 12px">
                {this.$t('Toplam Kayıt')}:<a style="font-weight: 600"> {this.localMeta.total}</a>
              </span>
              {needTotalItems}
              {clearItem}
            </template>
